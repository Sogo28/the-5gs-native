...
public class BackgroundRenderer {
    ...
    public FloatBuffer getCameraTexCoords() {
        return cameraTexCoords;
    }
     ...
     * during a {@link SampleRender.Renderer} callback, typically in [SampleRenderer.Renderer.onSurfaceCreated]
     */
    ...
    public void updateDisplayGeometry(Frame frame) {
        if (frame.hasDisplayGeometryChanged()) {
            NDC_QUAD_COORDS_BUFFER.rewind(); // Très important !
            cameraTexCoords.rewind();
            var expectedSize = 4 * 2;
            // If display rotation changed (also includes view size change), we need to re-query the UV
            // coordinates for the screen rect, as they may have changed as well.
            if (NDC_QUAD_COORDS_BUFFER.remaining() != expectedSize ||
                    cameraTexCoords.capacity() < expectedSize) {
                Log.w(TAG, "Buffers not ready yet, skipping UV update this frame.");
                return;
            }
